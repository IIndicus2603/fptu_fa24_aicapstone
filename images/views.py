...
@csrf_exempt
def wait_and_notify(file_name):
    # Check for AJAX request using X-Requested-With header
    file_path = os.path.join(settings.BASE_DIR, 'media','images',file_name)
...
def image_upload(request):
    if request.method == 'POST' and request.FILES.get('image'):
        ...
        if form.is_valid():
            ...
            tags = wait_and_notify(file_name)  # Notify loading status
            try:
                # Open and load existing tag data
                if os.path.exists(tag_file_path):
                    with open(tag_file_path, 'r') as f:
                        tag_data = json.load(f)
                    tag_data[file_name] = tags
